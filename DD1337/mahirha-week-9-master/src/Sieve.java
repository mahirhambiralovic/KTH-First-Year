...
public static void main(String[] args) {
    ...
    private boolean[] sieve(int number){
        ...
        if(primeCache.length > number){
            ...
            // Fill prime with primeCache values for reuse
            System.arraycopy(primeCache, 0, prime, 0, primeCache.length);
            // Fill remaining with true
            Arrays.fill(prime, primeCache.length, prime.length , true);
            ...
            for (int i = 2; i <= sqrt; i++) {
                ...
                if(primeCache.length == 0){
                    ...
                    // Check if i has been used in primeCache
                    if((int)(Math.floor((primeCache.length-1)/i)) > 0){
                        if (prime[i]) {
                            for (int j = (int)(Math.floor((primeCache.length-1)/i +1)) *i; j < prime.length; j += i) {
                                prime[j] = false; // mark multiples of i as not prime
                            }
                        }
                    }
