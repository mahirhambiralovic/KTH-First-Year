...
##### ANSWER:
There might be some potential issues in the following line of code:
```java
Lot selectedLot = lots.get(lotNumber - 1);
```

As lots.get(int) would call for a lot which does not exist, it would give a NullPointerException
...
For loop is easiest because we need to access the hour that we've counted on as well.
...
Currently it will give the first hour that had the greatest count.
E.g. if 13 and 18 both were busiest hours, it will return 13 as the busiest.
